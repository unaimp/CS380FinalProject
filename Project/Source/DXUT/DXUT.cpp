...
protected:
    struct STATE
    {
        ...
        ID3D10Counter*			m_Counter_FillrateThroughputUtilization;
...
public:
    ...
    GET_SET_ACCESSOR( ID3D10Counter*, Counter_FillrateThroughputUtilization );
...
DXUTState& GetDXUTState()
{
    ...
    // However the application can also call DXUTCreateState() & DXUTDestroyState() independently if its wants 
...
RECT WINAPI DXUTGetFullscreenClientRectAtModeChange()       { RECT rc = { 0, 0, GetDXUTState().GetFullScreenBackBufferWidthAtModeChange(), GetDXUTState().GetFullScreenBackBufferHeightAtModeChange() }; return rc; }
    ...
    switch( uMsg )
    {
       ...
       case WM_ACTIVATEAPP:
            if( wParam == TRUE && !DXUTIsActive() ) // Handle only if previously not active 
            {
                ...
                // The GetMinimizedWhileFullscreen() variable is used instead of !DXUTIsWindowed()
...
HRESULT DXUTDelayLoadDXGI()
{
    ...
    if( pDXGIFactory == NULL )
    {
        ...
        if( pDXGIFactory == NULL )
        {
            // If still NULL, then DXGI is not available
...
//--------------------------------------------------------------------------------------
// Allows the app to explicitly state if it supports D3D9 or D3D10.  Typically
...
//--------------------------------------------------------------------------------------
// Sets the viewport, creates a render target view, and depth stencil texture and view.
//--------------------------------------------------------------------------------------
HRESULT DXUTSetupD3D10Views( ID3D10Device* pd3dDevice, DXUTDeviceSettings* pDeviceSettings )
{
    HRESULT hr = S_OK;
...
void DXUTCreateD3D10Counters( ID3D10Device* pd3dDevice )
{
    ...
    if( SUCCEEDED( hr = pd3dDevice->CreateCounter( &Desc, &pCounter ) ) )
        GetDXUTState().SetCounter_FillrateThroughputUtilization( pCounter );
...
void DXUTDestroyD3D10Counters()
{
    ...
    pCounter = GetDXUTState().GetCounter_FillrateThroughputUtilization();
    SAFE_RELEASE( pCounter );
    GetDXUTState().SetCounter_FillrateThroughputUtilization( NULL );
...
void DXUTAllowShortcutKeys( bool bAllowKeys )
{
    ...
    if( bAllowKeys )
    {
        ...
        // Remove the keyboard hook when it isn't needed to prevent any slow down of other apps
...
HRESULT WINAPI DXUTToggleFullScreen()
{
    ...
    DXUTDeviceSettings deviceSettings = DXUTGetDeviceSettings();
    DXUTDeviceSettings originalDeviceSettings = DXUTGetDeviceSettings();
    ...
    if( SUCCEEDED(hr) ) 
    {
        ...
        if( FAILED(hr) && (hr != E_ABORT) )
        {
            ...
            HRESULT hr2 = DXUTChangeDevice( &originalDeviceSettings, NULL, NULL, false, false );
            if( FAILED(hr2) )
            {
