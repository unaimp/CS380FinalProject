...
//substate (which is equivalent to being scoped to the state if 
...
public:
	...
	bool operator==(const MSG_Object &rhs) const
	{
		if (Near(m_deliveryTime, rhs.m_deliveryTime) &&
			(m_name == rhs.m_name) &&
			(m_sender == rhs.m_sender) &&
			(m_receiver == rhs.m_receiver) &&
			(m_scope == rhs.m_scope) &&
			(m_scopeRule == rhs.m_scopeRule) &&
			(m_queue == rhs.m_queue) &&
			(m_timer == rhs.m_timer)
			)
			return true;
