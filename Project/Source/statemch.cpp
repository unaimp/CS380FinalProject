...
void StateMachine::PerformStateChanges( void )
{
	...
	while( m_stateChange != NO_STATE_CHANGE && (--safetyCount >= 0) )
	{
		...
		m_timeOnEnterSubstate = g_clock.GetCurTime();
		if( m_nextSubstate < 0 ) {
			m_timeOnEnterState = g_clock.GetCurTime();
		}
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Sends a message to all target game objects within a radius.
  
  Arguments:    radius               : the radius from the sender
				name                 : the name of the message
				targetGameObjectType : (optional) the type of object that should receive the message
				data                 : (optional) associated data to deliver with the message

  Returns:      None.
 *---------------------------------------------------------------------------*/
void StateMachine::SendMsgRadius( float radius, MSG_Name name, unsigned int targetGameObjectType, MSG_Data& data )
{
	g_trigger.SendMsgRadius( radius, ONE_FRAME, name, m_owner->GetID(), targetGameObjectType, data );
}

/*---------------------------------------------------------------------------*
  Name:         SendMsgRadiusNow

  Description:  Immediately sends a message to all target game objects within a radius.
  ...
  Returns:      None.
 *---------------------------------------------------------------------------*/
void StateMachine::SendMsgRadiusNow( float radius, MSG_Name name, unsigned int targetGameObjectType, MSG_Data& data )
{
	g_trigger.SendMsgRadius( radius, 0, name, m_owner->GetID(), targetGameObjectType, data );
}
...
/*---------------------------------------------------------------------------*
  Name:         SendMsgDelayedRadius

  Description:  Sends a message at a specified time in the future to all game
				objects within a given radius.
  
  Arguments:    delay                : the number of seconds in the future to deliver the message
				radius               : the radius from the sender
				name                 : the name of the message
				targetGameObjectType : (optional) the type of object that should receive the message
				data                 : (optional) associated data to deliver with the message
  ...
  Returns:      None.
 *---------------------------------------------------------------------------*/
void StateMachine::SendMsgDelayedRadius( float delay, float radius, MSG_Name name, unsigned int targetGameObjectType, MSG_Data& data )
{
	g_trigger.SendMsgRadius( radius, delay, name, m_owner->GetID(), targetGameObjectType, data );
}
...
void StateMachineManager::ProcessStateMachineChangeRequests( StateMachineQueue queue )
{
	...
	{
		ASSERTMSG( safetyCount > 0, "StateMachineManager::ProcessStateMachineChangeRequests - State machines are flip-flopping in an infinite loop." );
