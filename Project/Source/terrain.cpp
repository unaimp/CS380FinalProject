...
void Terrain::ResetMap(void)
{
	...
	if (player != nullptr)
	{
		player->GetQuoridor().SetWallCount(10);
		npc->GetQuoridor().SetWallCount(10);
		if (m_nextMap == 0)
		{
			player->GetQuoridor().SetPlayerInTile(0, 4);
			npc->GetQuoridor().SetPlayerInTile(8, 4);
...
void Terrain::ResetColors( void )
{
	for( int r=0; r<m_width; r++ )
	{
		for( int c=0; c<m_width; c++ )
		{
			...
			{
				m_terrainColor[r][c] = DEBUG_COLOR_GRAY;
			}
		}
	}
}
	...
	for( int r=0; r<m_width; r++ )
	{
		for( int c=0; c<m_width; c++ )
		{
			...
			case TILE_IN_BETWEEN:
				g_debugdrawing.DrawQuad(pd3dDevice, &pos, 1.0f/m_width, DEBUG_COLOR_GRAY);
				break;
