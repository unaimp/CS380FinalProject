...
protected:
	...
	//Triggers
	/////////////////////////////////////
	//Set trigger in world
	Trigger* SetTrigger( float radius, D3DXVECTOR3& pos, MSG_Name msgSendToMe, MSG_Name msgSendToOther, unsigned int gameObjectTypeToDetect, bool persistAfterFiring );
	//Set trigger attached to me
	Trigger* SetTriggerAttachedToMe( float radius, MSG_Name msgSendToMe, MSG_Name msgSendToOther, unsigned int gameObjectTypeToDetect, bool persistAfterFiring );
	//Delete trigger
	...
	//Send message to game objects within a particular radius of the sender
	void SendMsgRadius( float radius, MSG_Name name, unsigned int targetGameObjectType = OBJECT_Ignore_Type, MSG_Data& data = MSG_Data() );
	//Send message immediately to game objects within a particular radius of the sender
	void SendMsgRadiusNow( float radius, MSG_Name name, unsigned int targetGameObjectType = OBJECT_Ignore_Type, MSG_Data& data = MSG_Data() );
	...
	//Send message delayed to game objects within a particular radius (objects may not be within radius after delay)
	void SendMsgDelayedRadius( float delay, float radius, MSG_Name name, unsigned int targetGameObjectType = OBJECT_Ignore_Type, MSG_Data& data = MSG_Data() );
	...
	//Helper functions
	inline float GetTimeInState( void )					{ return( g_clock.GetCurTime() - m_timeOnEnterState ); }
	inline float GetTimeInSubstate( void )				{ return( g_clock.GetCurTime() - m_timeOnEnterSubstate ); }
